...
- `just build` builds `dist/`, minifies HTML/CSS/JS, copies public files, generates AVIF derivatives, and writes Brotli and gzip variants
...
- `just webm path/to/file.avif` converts AVIF files to WebM beside the source path
...
`bin/rsdeploy` reads the local `.env` file that already exists in the repo setup and uses `.rsdeploy-exclude` for the rsync filter.

If you need a template for local setup, copy `.env.example` to `.env` and fill in the destination.
